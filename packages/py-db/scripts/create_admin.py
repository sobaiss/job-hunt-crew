...
Every other way to become an Administrator (`PUT /v1/admin/users/{id}/plan`)
requires the caller to already be one — this script is how the first account
is created, since there's no other path in. Also works to create additional
Administrators later, or to (re)set an existing User's password.
