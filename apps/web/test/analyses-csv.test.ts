...
describe("analysesToCsv", () => {
  it("emits a header row plus one row per analysis, in the given order", () => {
    ...
    expect(lines[0]).toBe("Position,Company,Platform,Posted,CV,Score,Status,Link");
  ...
  it("resolves the platform, score, status and link for a COMPLETED analysis", () => {
    ...
    const [, row] = csv.split("\n");
    expect(row).toBe(
      "Backend Engineer,\"Acme, Inc\",France Travail,2026-07-01T00:00:00.000Z,Grad CV,87,To apply,https://example.com/jobs/job1",
    );
  });
  ...
  it("escapes commas, quotes and newlines in a field", () => {
    ...
    const [, row] = csv.split("\n");
    expect(row.startsWith('"Say ""hi"", please",')).toBe(true);
  });
});
