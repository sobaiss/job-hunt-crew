...
**Dashboard**:
The signed-in overview at `/` — stat tiles (average and best Match score, analysis count, CV-version count), recent analyses, a Match-score trend, and quick actions. Every number but the CV count comes from `GET /api/analyses/stats`, which answers over *every* Analysis the Candidate has; they used to be derived here from the full Analyses list, which stopped being something to fetch when that list became one page at a time (docs/adr/0033). While the Candidate has no Analysis yet it shows a three-step onboarding checklist in place of the tiles. Where `/` sends a Candidate once they have a Session.
_Avoid_: Home, Overview (fine in prose; the term is Dashboard), analyses list (that's Analyses)

**Analyses**:
The list at `/analyses` — one row per Analysis (no SITE_SEARCH batch grouping; see [API](../../services/api/CONTEXT.md)'s Analysis batch, which this view deliberately no longer folds rows into), as a sortable, filterable table with Column visibility control: a search (title/company), a location search, a status filter (Tracking status plus the pipeline buckets `PENDING`/`FAILED` — see Tracking status below), a platform filter, and a `requestedAt` date-range filter, plus date/score/company/platform sort and multi-select bulk actions. All of that is applied **server-side** (docs/adr/0033) — the list is fetched one page at a time, narrowed and ordered by `GET /api/analyses`, where it used to arrive whole and be narrowed in the browser. Two consequences a reader will notice: the selection is the current page's and empties when the page changes (so the CSV export covers at most one page, and there is no "select all matching your filters"), and the two free-text boxes commit on a pause rather than on every keystroke, with the table holding the previous page on screen while the next loads. Status and platform each take several values at once, as a checkbox menu: values inside one filter are OR-ed, the filters AND-ed, an empty selection means no filter, and the URL carries them comma-separated (`?status=TO_APPLY,FAILED`), which still reads a single-value link minted before that. The "Plus de filtres" toggle's count stays a count of *filters* — two platforms ticked count as one, since the number says how much is hiding inside the closed panel, not how many boxes are ticked. Clicking a row opens its detail in a right-hand slide-over without leaving the list; the slide-over links out to the full Analysis detail page and the Side-by-side comparison for deeper reading, and steps from one Analysis to the next itself (see Quick view navigation). Reached from the Dashboard and the Sidebar.
...
The alert block on the Analysis detail page and in the Quick view when the API reports an Analysis as stuck — "Cette analyse semble interrompue…" plus a **Reprendre l'analyse** button ("Pick it up again"). That button used to read "Relancer l'analyse", the very words the *other* action carries (see Reprise vs Relance below); it was renamed once a candidate reported the two as one button that worked in the Quick view and stayed dead in the bulk bar. The condition is read straight off the API's `stuck` field, never re-derived from timestamps here, so the threshold stays one server-side setting and the endpoint's own re-check cannot disagree with the button that led to it (see [API](../../services/api/CONTEXT.md)'s Stuck). The Analyses-list bulk-actions bar carries the same repair over a selection, labelled by how many ("Reprendre 2 analyses interrompues") and offered on any selection: at none it says "Aucune analyse interrompue dans la sélection" rather than hiding, which used to leave the repair discoverable only by candidates who already knew it existed — and a restart strands rows by the dozen. It posts to `POST /api/analyses/{id}/requeue`, which re-drives the same Analysis and spends no quota — so the block deliberately does *not* carry the quota warning Relancer l'analyse shows on a terminal Analysis. A `409` comes back as "Cette analyse est en fait toujours en cours de traitement", since the server re-checks and the row may have been picked up between the read and the click. While it shows, the "Cette analyse est toujours en cours" note is suppressed — both are true of a non-terminal row and together they contradict each other. The Analyses list does not poll on its own, so the block appears there after an Actualiser; the detail page polls and reveals it without being asked.
...
**Reprise vs Relance** (of an Analysis):
Two different acts that French copy called by one name until a candidate tripped over it. A **Reprise** ("Reprendre l'analyse") re-drives the *same* interrupted row for free — [API](../../services/api/CONTEXT.md)'s Requeue, whose own _Avoid_ list has named Relaunch as the wrong word for it all along. A **Relance** ("Relancer l'analyse") creates a *new* Analysis and spends a daily quota — [API](../../services/api/CONTEXT.md)'s Re-run. They apply to disjoint rows (a Reprise needs a non-terminal stuck row, a Relance a `COMPLETED`/`FAILED` one), which is exactly why sharing a label was invisible in the Quick view — only one of the two is ever offered there — and glaring in the bulk-actions bar, where both buttons sit side by side and one is inert on any given selection. Neither button hides when it has nothing to act on: each says so, the Relance in its confirmation, the Reprise as a plain notice.
_Avoid_: Retry (ambiguous between the two — it is the English copy for the Relance and the word the API glossary warns off the Reprise), Redémarrer, Resume (the pipeline restarts from the top; it does not continue mid-step)
