...
**Stuck** (an Analysis):
A non-terminal Analysis nothing is going to advance, because the worker or ElasticMQ restarted and took its queued `analysis-intake` message with it (the local queue is in-memory only). Derived on read, never stored, by `py_db/stuck_analysis.py`'s `is_analysis_stuck` and exposed as `AnalysisResponse.stuck` so the Web reads a boolean instead of re-deriving one: non-terminal, nothing newer than `max(requestedAt, requeuedAt, startedAt)` for `ANALYSIS_STUCK_AFTER_MINUTES`, **and** either the row had already started or the whole pipeline has been quiet for `PIPELINE_IDLE_GRACE_MINUTES`. That last term is the point of the rule, not a detail: a `PENDING` row may legitimately be queued behind a Scout's fan-out (25 offers per site, drained one at a time), so the newest `PipelineEvent` across the pipeline is used as a liveness probe — while anything is still moving, a queued row is waiting rather than lost. The probe is global, not per-Analysis, exactly because the row being judged has no events of its own. Resolved once per request and passed into `_analysis_response`, never once per row.
