...
async def test_start_analysis_workflow_records_execution_arn_and_queues():
    ...
    stub_client = StubSfnClient(
        execution_arn="arn:aws:states:us-east-1:1:execution:AnalysisWorkflow:x"
    )
    ...
    try:
        ...
        assert (
            ...
            == "arn:aws:states:us-east-1:1:stateMachine:AnalysisWorkflow"
        )
        ...
        async with session_factory() as session:
            ...
            assert (
                reloaded.stepFunctionExecutionArn
                == "arn:aws:states:us-east-1:1:execution:AnalysisWorkflow:x"
            )
    finally:
        await _cleanup(
            engine, session_factory, user_id, job_offer_id, cv_version_id, analysis_id
...
@pytest.mark.asyncio
async def test_start_analysis_workflow_raises_when_state_machine_arn_not_configured(
    monkeypatch,
):
    ...
    try:
        async with session_factory() as session:
            with pytest.raises(IntakeError):
                await start_analysis_workflow(
                    session, analysis_id, sfn_client=stub_client
                )
        assert stub_client.start_execution_calls == []
    finally:
        await _cleanup(
