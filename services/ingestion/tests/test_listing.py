...
def _page_html(page: int, *, has_next: bool) -> str:
    next_link = (
        f'<a rel="next" href="{_page_url(page + 1)}">Next</a>' if has_next else ""
    )
...
async def test_fetch_listing_pages_caps_at_default_max_pages():
    with respx.mock(assert_all_called=False) as mock:
        mock.get(_page_url(1)).mock(
            return_value=Response(200, text=_page_html(1, has_next=True))
        )
        mock.get(_page_url(2)).mock(
            return_value=Response(200, text=_page_html(2, has_next=True))
        )
        mock.get(_page_url(3)).mock(
            return_value=Response(200, text=_page_html(3, has_next=True))
        )
        mock.get(_page_url(4)).mock(
            return_value=Response(200, text=_page_html(4, has_next=True))
        )
        mock.get(_page_url(5)).mock(
            return_value=Response(200, text=_page_html(5, has_next=False))
        )
...
async def test_fetch_listing_pages_stops_early_when_no_next_link():
    with respx.mock(assert_all_called=True) as mock:
        ...
        mock.get(_page_url(2)).mock(
            return_value=Response(200, text=_page_html(2, has_next=False))
        )
