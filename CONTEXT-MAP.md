...
- **API → Analysis**: `POST /v1/analyses` sends an SQS message that Analysis's intake handler consumes to start an AnalysisWorkflow execution. `POST /v1/cv-versions/{id}/convert` likewise sends an SQS message that an Analysis handler consumes to build that CVVersion's Markdown rendition (Conversion).
